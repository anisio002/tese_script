...
for data in listaDados:
    ...
    for name, model in pipelines:
        with open(file_path +'Split_'+ name + '_T_' + city + '.csv', 'a', newline='') as file:
    ...
    for test_size in training_sets:
        ...
        for name, model in pipelines:
            file_name = file_path +'Split_'+ name + '_T_' + city
            results=[]
            X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=seed)
            X_f_train, X_f_test, y_f_train, y_f_test= train_test_split(X_o, y, test_size=test_size, random_state=seed)
            ...
            dataY = {'y_test': y_test, 'y_pred': y_pred.ravel()}
            dfY = pd.DataFrame(dataY)
            dfY.to_csv(file_name + f'{str(test_size)}_Y_.csv', index=False)
            mlp_time = fim - inicio
            print(f'Testsize: {str(test_size)} - Localidade: {city}')
            results.append([city, str(test_size), sqrt(mean_squared_error(y_test, y_pred)), mlp_time, num_epochs, train_score[0],train_score[1],test_score[0],test_score[1]])
